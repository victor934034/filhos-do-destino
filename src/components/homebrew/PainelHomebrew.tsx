...
import type { Monstro, ItemCompendio, PoderCompendio } from "@prisma/client";
...
/**
 * As 3 listas já vêm prontas do servidor (uma única leva, no carregamento da página) —
 * trocar de aba aqui é só troca de estado local, sem voltar no banco a cada clique
 * (antes cada aba era uma navegação via ?tab=, refazendo as 3 consultas de novo).
 */
export function PainelHomebrew({
  monstros,
  itens,
  poderes,
}: {
  monstros: Monstro[];
  itens: ItemCompendio[];
  poderes: PoderCompendio[];
}) {
  const [aba, setAba] = useState<Aba>("monstros");
